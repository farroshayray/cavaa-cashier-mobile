...
class ReportApi {
  ...
  Future<Map<String, dynamic>> getSoldProducts({
    required String from,
    required String to,
    required String cashierScope,
    List<String> paymentFilters = const [],
    ...
    final resp = await dio.get(
      '/api/v1/mobile/cashier/reports/sold-products',
      queryParameters: {
        'from': from,
        'to': to,
        'cashier_scope': cashierScope,
  ...
  Future<Map<String, dynamic>> getTransactionDetail({required int id}) async {
    final resp = await dio.get(
      '/api/v1/mobile/cashier/reports/transactions/$id',
    );
